...
usage() {
  cat <<'EOF'
Usage:
  run_depth_pro_flops.sh [options]
...
"${DOCKER[@]}" run --rm --runtime=nvidia \
  ...
  "$IMAGE" \
  bash -lc "export PYTHONPATH=/workspace/model/src:\$PYTHONPATH && python3 /workspace/runner/scripts/jetson/depth_pro_flops_probe.py --model-root /workspace/model --input-image /workspace/input/$(basename "$input_image") --output-json /workspace/output/$(basename "$OUTPUT_JSON")"
