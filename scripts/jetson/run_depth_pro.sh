...
MODEL_ROOT="$WORKSPACE_ROOT/external_models/ml-depth-pro"
OUTPUT_ROOT="$WORKSPACE_ROOT/artifacts/depth_pro"
...
for ds in "${datasets[@]}"; do
  ...
  echo
  echo "== DepthPro dataset=$ds input=$(basename "$input_dir") limit=$LIMIT =="
  "${DOCKER[@]}" run --rm --runtime=nvidia \
    ...
    "$IMAGE" \
    bash -lc "export PYTHONPATH=/workspace/model/src:\$PYTHONPATH && python3 /workspace/runner/scripts/jetson/depth_pro_batch.py --model-root /workspace/model --input-dir /workspace/input --output-dir /workspace/output ${limit_arg[*]} ${progress_arg[*]}"
done
